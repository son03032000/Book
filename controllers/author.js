...
try {
  ...
  .sort([['last_name', 'ascending']])
    if (list_authors) { 
      return res.render('author/author_list', { title: 'Author List', author_list: list_authors });
    };
} catch (error) {
...
exports.author_detail =  (req, res, next) => {
  ...
  res.render('author/author_detail', { title: 'Author Detail', author: data.author, author_books: data.authors_books });
});
}
...
exports.author_create_post = (req, res) => {
    ...
    req.checkBody('last_name', 'last name must be Alphanumeric.').isAlphanumeric();
    ...
    if (errors) {
        res.render('author/author_form', { title: 'Create Author', author: author, errors: errors});
    return;
    }
    else {
...
exports.author_delete_get = function(req, res, next) {
    async.parallel({
      ...
      if (err) { return next(err); }
      if (data.author == null) { // No data.
          res.redirect('/catalog/authors');
      }
      //Successful, so render
      res.render('author/author_delete', {title: 'Delete Author', author: data.author, author_books: data.authors_books});
    });
};
