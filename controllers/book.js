...
exports.index = function(req, res){  // hiện Thư viện có số lượng bản ghi sau
...
exports.book_list = function(req, res, next) {
    ...
    Book.find({}, 'title author')
      ...
      .exec(function(err, list_books){
        ...
        res.render('books/book_list', {title: 'Book List', book_list: list_books}); //(có thể dùng ajax)
      });
};
...
exports.book_detail = function(req, res, next) {
    async.parallel({
      ...
      //Successful, so render
      res.render('books/book_detail', {title: 'Title', book: data.book, book_instances: data.book_instance });
    });
};
...
exports.book_create_get = function(req, res, next) {
    ...
    async.parallel({
      ...
      res.render('books/book_form', {title: 'Create Book', authors: data.authors, genres: data.genres});
    });
};
...
exports.book_create_post = async (req, res, next) => {
  ...
  // Nhận tất cả các tác giả và thể loại cho biểu mẫu
    async.parallel({
      ...
      res.render('books/book_form', {title: 'Create Book', authors: data.authors, genres:data.genres, book: book, errors:errors});
    });
...
exports.book_delete_get = function(req, res, next) {
    async.parallel({
      ...
      if(err){ return next(err); }
      //Successful
      res.render('books/book_delete', {title: 'Delete Book', book: data.book, book_bookinstances: data.book_bookinstances});
    });
};
...
exports.book_delete_post = function(req, res) {
  ...
  async.parallel({
    ...
    if(data.book_bookinstances.length > 0){
      ...
      res.render('books/book_delete', {title: 'Delete Book', book: data.book, book_instances: data.book_bookinstances});
      return;
    }
    else{
...
exports.book_update_get = function(req, res, next) {
    ...
    async.parallel({
        book: function(data) {
            Book.findById(req.params.id).populate('author').populate('genre').exec(data);
        },
        authors: function(data) {
            Author.find(data);
        },
        genres: function(data) {
            ...
            for (var all_g_iter = 0; all_g_iter < data.genres.length; all_g_iter++) {
                for (var book_g_iter = 0; book_g_iter < data.book.genre.length; book_g_iter++) {
                    if (data.genres[all_g_iter]._id.toString()==data.book.genre[book_g_iter]._id.toString()) {
                        data.genres[all_g_iter].checked='true';
                    }
                }
            }
            res.render('books/book_form', { title: 'Update Book', authors:data.authors, genres:data.genres, book: data.book });
        });
    ...
    async (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('books/book_form', { title: 'Update Book',authors:data.authors, genres:data.genres, book: book, errors: errors.array() });
            });
            return;
        }
        else {
